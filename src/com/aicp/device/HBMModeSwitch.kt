...
class HBMModeSwitch(context: Context) : OnPreferenceChangeListener {
    ...
    fun onPreferenceChange(preference: Preference?, newValue: Any): Boolean {
        ...
        Utils.writeValue(getFile(mContext), if (enabled) mHBMOnState else mHBMOffState)
        return true
    }

    companion object {
        val SETTINGS_KEY: String = DeviceSettings.Companion.KEY_SETTINGS_PREFIX + DeviceSettings.Companion.KEY_HBM_SWITCH
        fun getFile(context: Context): String? {
            val fileName: String = context.getResources().getString(R.string.pathHBMModeToggle)
